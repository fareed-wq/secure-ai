...
from api.auth.entitlements import require_current_user, Entitlements
...
def test_create_share_success(mock_entitlements_free, mock_db_env):
    app.dependency_overrides[require_current_user] = lambda: {'sub': 'user-123'}
    try:
...
def test_create_share_race_condition(mock_entitlements_free, mock_db_env):
    app.dependency_overrides[require_current_user] = lambda: {'sub': 'user-123'}
    try:
        with patch('api.index.requests.get') as mock_get, \
             patch('api.index.requests.post') as mock_post:
            ...
            mock_get.side_effect = [
                MagicMock(status_code=200, json=lambda: [{"id": "scan-123"}]),
                MagicMock(status_code=200, json=lambda: []),
                MagicMock(status_code=200, json=lambda: [{"share_token": "race-token", "created_at": "2023-01-01T00:00:00Z"}])
            ]
            ...
            response = client.post("/api/share/create", json={"scan_id": "scan-123"})
            assert response.status_code == 200
            assert response.json() == {"share_token": "race-token", "created_at": "2023-01-01T00:00:00Z"}
    finally:
        app.dependency_overrides.clear()
...
def test_revoke_share_success(mock_entitlements_free, mock_db_env):
    ...
    try:
        with patch('api.index.requests.patch') as mock_patch:
            mock_patch.return_value = MagicMock(status_code=200, json=lambda: [{"id": "share-1"}])
...
def test_revoke_share_zero_rows(mock_entitlements_free, mock_db_env):
    app.dependency_overrides[require_current_user] = lambda: {'sub': 'user-123'}
    try:
        with patch('api.index.requests.patch') as mock_patch:
            ...
            response = client.post("/api/share/revoke", json={"share_token": "mock-token"})
            assert response.status_code == 404
            assert response.json()["error"] == "Share link unavailable."
    finally:
        app.dependency_overrides.clear()
...
def test_get_shared_report_success(mock_db_env):
    with patch('api.index.requests.get') as mock_get:
        mock_get.side_effect = [
            ...
            MagicMock(status_code=200, json=lambda: [{"target_url": "example.com", "score": 95, "report_data": {"url": "example.com", "internal_secret": "hidden", "user_id": "private-uuid"}, "created_at": "2023-01-01"}])
        ]
        ...
        assert "internal_secret" not in data["report_data"]
        assert "user_id" not in data["report_data"]
