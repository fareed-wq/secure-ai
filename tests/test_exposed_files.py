...
def test_sql_dump_vs_zip_dump(module, monkeypatch):
    ...
    assert sql_finding["severity"] == "Medium"
    assert "full disclosure of stored application data" in sql_finding.get("impact", "")
    
    zip_finding = next((f for f in findings if "Exposed Backup / Archive File" in f["name"]), None)
    assert zip_finding is not None
    assert zip_finding["severity"] == "Medium"
    assert "may expose application files" in zip_finding.get("impact", "")
