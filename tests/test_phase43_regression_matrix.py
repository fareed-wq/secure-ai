...
class TestRegressionMatrix:
    ...
    def test_security_txt(self, mock_safe_req):
        ...
        mock_safe_req.return_value = make_mock_response(status_code=200, headers={"Content-Type": "text/plain"}, text="Contact: security@example.com\nExpires: 2030-12-31T23:59:59Z")
        f = mod.run("https://example.com", "example.com", MagicMock())
        assert any("Valid security.txt" in x["name"] for x in f)
        
        mock_safe_req.return_value = make_mock_response(status_code=404)
        f = mod.run("https://example.com", "example.com", MagicMock())
