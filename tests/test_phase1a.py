...
def test_legacy_string_evidence():
    mod = DummyModule()
    finding = mod.make_finding("Test", "Low", "Desc", "A long string" * 50)
...
def test_structured_dict_evidence():
    mod = DummyModule()
    finding = mod.make_finding("Test", "Low", "Desc", {"method": "GET"}, verification_state=VerificationState.OBSERVED)
    assert finding["evidence"]["method"] == "GET"
    assert finding["state"] == "Observed"

def test_nested_evidence():
    mod = DummyModule()
    finding = mod.make_finding("Test", "Low", "Desc", {"req": {"headers": {"host": "example.com"}}})
...
def test_redaction_in_structured_evidence():
    ...
    finding = mod.make_finding("Test", "Low", "Desc", {"req": "AKIAIOSFODNN7EXAMPLE", "arr": ["sk_live_12345"]})
...
def test_clipping_in_structured_evidence():
    mod = DummyModule()
    long_str = "A" * 1000
    finding = mod.make_finding("Test", "Low", "Desc", {"short": "B", "long": long_str, "nested": {"deep": long_str}, "arr": [long_str]})
...
def test_states():
    mod = DummyModule()
    f1 = mod.make_finding("Test", "Low", "Desc", "Ev", verification_state=VerificationState.OBSERVED)
    f2 = mod.make_finding("Test", "Low", "Desc", "Ev", verification_state=VerificationState.INFERRED)
    f3 = mod.make_finding("Test", "Low", "Desc", "Ev", verification_state=VerificationState.NOT_VERIFIED)
...
def test_omitted_verification_state():
    mod = DummyModule()
    f1 = mod.make_finding("Test", "Low", "Desc", "Ev")
    assert "state" in f1
    assert f1["state"] is None

def test_rule_id_and_instance_key_preserved():
    mod = DummyModule()
    f1 = mod.make_finding("Test", "Low", "Desc", "Ev", rule_id="rule1", instance_key="inst1")
