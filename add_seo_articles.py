...
new_articles = [
    {
        ...
        'sections': [
            ...
            },
            {
                ...
                'content': 'You can add the HSTS header at your web server, load balancer, or CDN level. Below are basic examples for common web servers. <strong>Warning:</strong> Start with a short max-age (like 5 minutes) to test your configuration before committing to a 1-year duration.',
    ...
    },
    {
        ...
        'sections': [
            ...
            },
            {
                ...
                'content': 'While X-Frame-Options is highly recommended for legacy support, modern browsers prefer the rame-ancestors directive within the Content-Security-Policy header, which allows for more complex framing rules.',
        ...
        'sections': [
            {
                ...
                'content': 'Developers often use Access-Control-Allow-Origin: * to bypass frustrating browser errors during development. However, leaving this in production allows any malicious website on the internet to make authenticated API requests to your server and read the responses.',
        ...
        'sections': [
            {
                ...
                'list': [
                    ...
                    'Requires zero specialized authorization to run'
                ]
            },
            {
                'id': 'active-testing',
...
with open('src/data/blog/index.js', 'r', encoding='utf-8') as f:
    content = f.read()

# We need to insert the new_articles into the 'articles' array in index.js.
# It ends with '];' or '];\n'
import re

new_json_str = json.dumps(new_articles, indent=4)
# Remove the brackets of the new JSON array
new_json_str = new_json_str.strip()[1:-1]

# Find the last ']' in the file
last_bracket_idx = content.rfind(']')

if last_bracket_idx != -1:
    # Check if there's already items in the array, add a comma
    before = content[:last_bracket_idx].rstrip()
    if before.endswith('{') or before.endswith('['):
        # Empty array
        updated_content = before + new_json_str + content[last_bracket_idx:]
    else:
        updated_content = before + ',\n' + new_json_str + '\n' + content[last_bracket_idx:]
    
    with open('src/data/blog/index.js', 'w', encoding='utf-8') as f:
        f.write(updated_content)
    print('Successfully appended 6 articles.')
else:
